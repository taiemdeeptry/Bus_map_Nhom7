...
import UIT.IE303.BUSMAP.config.LevelExtend;
import UIT.IE303.BUSMAP.config.Speed;
import UIT.IE303.BUSMAP.data.BusStopDAO;
import UIT.IE303.BUSMAP.data.RouteDAO;
import UIT.IE303.BUSMAP.data.StationDAO;
import UIT.IE303.BUSMAP.entities.Address;
import UIT.IE303.BUSMAP.component.BusDistance;
import UIT.IE303.BUSMAP.component.BusStopRaw;
import UIT.IE303.BUSMAP.component.Result;
import UIT.IE303.BUSMAP.component.ResultRoute;
import UIT.IE303.BUSMAP.entities.Station;
import UIT.IE303.BUSMAP.config.MoveType;

public class Support {
    // hàm chuyển chuỗi từ nhiều ký tự sang dạng ...
    // Ví dụ "Đại học Công Nghệ Thông Tin - Đại học Quốc Gia thành phố Hồ Chí Minh"
    ...
    // Cách di chuyển bằng bus, hay walk được định nghĩa trong package config trong
    // lớp Speed
    ...
    // Hàm tính giời gian di chuyển trong một khoảng meter với vận tốc là speed đầu
    // vào
    ...
    // Lý do của việc đem thuật toán ra đây là để khi nào cập nhật thuật toán thì
    // chỉ cần mở file này lên edit

    // Hàm này tính kết quả từ điểm đến, điểm đi và số lượng tuyến đi qua
    // Đầu tiên lấy các station xung quanh của cả 2 điểm, sau đó tính ra các tuyến
    // đi qua gần 2 điểm đó
    ...
    public static List<Result> calculateResults(Context context, Address from, Address to, int route_amount) {
        ...
        // Gọi hàm calculate phía dưới để tính ra 2 dictionary chứa các route đi qua gần
        // 2 điểm này
    ...
    public static List<Result> findResults(Context context, Dictionary<String, BusDistance> dictionary_starts,
        ...
        while (enumeration.hasMoreElements()) {
            ...
            if (dictionary_ends.get(current_route) != null
                    && dictionary_starts.get(current_route).getBus_stop_raw().getOrder() < dictionary_ends
                            .get(current_route).getBus_stop_raw().getOrder()) {
    ...
    /*
     * Hàm duyệt qua tất cả các stations và lần lượt lấy được tất cá các
     * bus_stop_raw
     * Sau đó ta kiểm tra trong dictionary kết quả xem có đã có chứa route mà
     * bus_stop_raw đang chứa hay không
     * Nếu chưa thì add một item của dictionary
     * Nếu rồi thì ta sẽ kiểm tra xem khoảng cách của station đó có bé hơn khoảng
     * cách của station cũ hay không để cập nhật
     */
    /*
